<template>
  <article class="pt-8 px-5">

    <div class="maxw-10 bb-7 pb-6 mb-4">
      <h1 class="t-4 maxw-9 tw-semibold mb-4">A Flexible, Functional CSS Library for Building Literally Anything</h1>
      <p class="t-1">Designed to be as flexible and configurable as possible, Decent can be used for building anything from prototypes to production applications and websites.</p>
    </div>

    <div>

      <div>
        <p class="t-3 tw-semibold">16<span class="t-1">kb</span></p>
        <p>Minified and gzipped</p>
      </div>
    </div>

    <!--
      - Makes designing in the browser possible.
      - 16kb minified and gzipped.

      - Why functional works:
        - modularization of components.
        - class binding for interactions.
    -->
  </article>
</template>

<script>
export default {
  components: {
  }
}
</script>
